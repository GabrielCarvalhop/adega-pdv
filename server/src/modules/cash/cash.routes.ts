...
cashRouter.post('/:id/movements', validateBody(movementSchema), async (req, res) => {
  ...
  res
    .status(201)
    .json(await service.addMovement(req.auth!.tenantId, id, req.body, req.auth!.userId));
});

cashRouter.post('/:id/close', validateBody(closeSchema), async (req, res) => {
  const id = Number(req.params.id);
  if (Number.isNaN(id)) throw new AppError('ID inválido');
  res.json(await service.close(req.auth!.tenantId, id, req.body, req.auth!.userId));
});

const reopenSchema = z.object({ reason: z.string().min(1) });

cashRouter.post('/:id/reopen', requireRole('admin'), validateBody(reopenSchema), async (req, res) => {
  const id = Number(req.params.id);
  if (Number.isNaN(id)) throw new AppError('ID inválido');
