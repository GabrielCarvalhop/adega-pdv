  ...
  return (
    <Modal title="Fechar caixa" onClose={onClose}>
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {summary && (
          <div className="rounded-lg bg-neutral-50 p-3 text-sm">
            ...
            <div className="grid grid-cols-2 gap-x-4 gap-y-0.5 text-neutral-600">
              <span>Dinheiro</span>
              <span className="text-right">{formatBRL(summary.paymentBreakdown.dinheiro)}</span>
              <span>Pix</span>
              <span className="text-right">{formatBRL(summary.paymentBreakdown.pix)}</span>
              <span>Débito</span>
              <span className="text-right">{formatBRL(summary.paymentBreakdown.debito)}</span>
              <span>Crédito</span>
              <span className="text-right">{formatBRL(summary.paymentBreakdown.credito)}</span>
            </div>
            {(summary.suprimentosCents > 0 || summary.sangriasCents > 0 || summary.canceledCount > 0) && (
              <div className="mt-1 border-t border-neutral-200 pt-1 text-xs text-neutral-500">
        ...
        <div>
          <label className="block text-sm font-medium text-neutral-700">Valor contado (R$)</label>
          <input
            value={counted}
